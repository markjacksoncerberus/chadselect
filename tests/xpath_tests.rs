...
fn extract_multiple_paragraphs() {
    let cs = make_cs();
    let results = cs.query(-1, "xpath://p/text()");
...
fn xpath_with_normalize_space_postprocess() {
    ...
    let results = cs.query(-1, "xpath://span[@class='price']/text() >> normalize-space()");
...
fn xpath_with_substring_after() {
    ...
    let results = cs.query(-1, r#"xpath://div[@class='vin']/text() >> substring-after('VIN: ')"#);
...
fn xpath_with_substring_before() {
    ...
    let results = cs.query(-1, r#"xpath://div[@class='info']/text() >> substring-before(': ')"#);
...
fn xpath_normalize_space_vs_trim() {
    ...
    let normalized = cs.query(-1, "xpath://p[@class='description']/text() >> normalize-space()");
    assert_eq!(normalized, vec!["This is a great vehicle!"]);

    let trimmed = cs.query(-1, "xpath://p[@class='description']/text() >> trim()");
...
fn invalid_xpath_returns_empty() {
    let cs = make_cs();
    let results = cs.query(-1, "xpath:[[[invalid");
...
fn xpath_no_match_returns_empty() {
    let cs = make_cs();
    let results = cs.query(-1, "xpath://nonexistent/text()");
    assert!(results.is_empty());
}
...
fn xpath_union_operator_works_without_functions() {
    ...
    let results = cs.query(-1, "xpath://span[@id='vin']/text() | //span[@id='stock']/text()");
...
fn xpath_index_selection() {
    ...
    let first = cs.query(0, "xpath://p/text()");
    assert_eq!(first, vec!["First paragraph"]);

    let second = cs.query(1, "xpath://p/text()");
